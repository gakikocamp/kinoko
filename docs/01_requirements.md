...
| F1-3 | MFA(多要素認証)の強制 | Must | TOTP認証アプリ。§08参照 |
| F1-4 | 管理者による新規ユーザー招待(自己サインアップなし) | Should | 当面は管理者1名 |
| F1-5 | ロール別権限(admin / staff / viewer) | Could | DBには role カラムを最初から持たせる(§02・§08権限マトリクス) |
...
### F8. 仕向国コンプライアンス(詳細は §07)

| ID | 機能 | 優先度 | 備考 |
|---|---|---|---|
| F8-1 | 国マスタ(輸出可🟢/条件付き🟡/対応不可🔴/未確認⚪+要件チェックリスト+最終確認日) | Must | 全196カ国をシード投入。未分類は⚪ |
| F8-2 | 顧客・案件の国選択時に可否バッジ表示 | Must | |
| F8-3 | 🔴の国への案件作成ブロック/⚪の国へのPI発行ブロック | Must | |
| F8-4 | 🟡の国: 要件チェックリストを案件に自動添付、未完了で出荷準備時に警告 | Must | deal_compliance_checks |
| F8-5 | 制裁リスト確認チェック(顧客登録時・手動運用) | Should | 将来はスクリーニングAPI連携 |

### F9. 監査・セキュリティ(詳細は §08)

| ID | 機能 | 優先度 | 備考 |
|---|---|---|---|
| F9-1 | 監査ログ(ログイン・書類発行・設定変更・権限変更等) | Must | INSERTのみ許可 |
| F9-2 | 銀行/Wise情報変更時の全管理者への即時通知 | Must | BEC(振込先すり替え)対策 |
| F9-3 | 発行PDFのSHA-256ハッシュ記録(改ざん検知) | Should | |

### F10. 設定

| ID | 機能 | 優先度 | 備考 |
|---|---|---|---|
| F10-1 | 自社(Exporter)情報: 社名/住所/電話/Email/代表者名 | Must | 全PDFのヘッダに使用 |
| F10-2 | 銀行送金情報・Wise受取情報の登録 | Must | PIに記載 |
| F10-3 | 署名画像・ロゴ画像のアップロード | Must | PDF署名欄・ヘッダに使用 |
| F10-4 | デフォルト通貨・デフォルト支払条件 | Should | |

---

## 2. 画面一覧

全20画面(MVP必須は #1〜#18)。画面設計の方針・UXパターンは [06_uiux_design.md](./06_uiux_design.md) を参照。
...
| 16 | 国・輸出ルール一覧 | `/countries` | 可否ステータス・最終確認日(1年超は要再確認表示)・検索 |
| 17 | 国詳細/編集 | `/countries/[code]` | ステータス変更(理由必須)・要件チェックリスト編集 |
| 18 | MFA設定 | `/settings/mfa` | 認証アプリ登録(初回ログイン時に強制) |
| 19 | ユーザー管理 | `/settings/users` | (Should) 招待・ロール変更 |
| 20 | 加工費テンプレート管理 | `/settings/fee-templates` | (Should) 定型説明文の管理 |
